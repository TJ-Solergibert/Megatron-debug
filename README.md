...
In the forward pass, for each ParallelTransformerLayer we will perform the following computations:
1. Normalize the input data with the `FastLayerNormFN` layer from the Apex package
2. Compute the Self-Attention + Linear layer. We will start with a input tensor of dimension `[512, 4, 1024]` (`[sequence length, batch size, hidden size]`). First, the `ColumnParallelLinear` layer will multiply this tensor with the matrix Q, K, and V of dimension `[1024, 3072]`, producing an output of dimension `[512, 4, 3072]`. Then, we divide the last dimension by the number of attention heads (in this example, 16) and obtain a tensor of `[512, 4, 16, 192]`, which we will split into 3 different tensors, each of shape `[512, 4, 16, 64]` containing the values of Q, K, and V. To compute the attention, first we will reshape the tensors Q and K to `[64, 512, 64]` and `[64, 64, 512]` respectively, and perform a batch matrix-matrix product of matrices ([torch.baddbmm](https://pytorch.org/docs/stable/generated/torch.baddbmm.html)), obtaining a tensor of dimensions `[64, 512, 512]`. We will reshape this tensor to `[4, 16, 512, 512]`, apply the scale_mask_softmax kernel and dropout to then convert it back to `[64, 512, 512]`. Finally, we perform a batch matrix-matrix product of matrices([torch.bmm](https://pytorch.org/docs/stable/generated/torch.bmm.html)) of this tensor with the reshaped + transposed V (`[512, 4, 16, 192]` ➡️ `[64, 512, 64]`): The result will be a tensor `[64, 512, 64]` that we will reshape back to `[512, 4, 1024]`.
